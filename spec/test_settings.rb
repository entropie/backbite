...
EOF
  ...
  it "it should parse the config string to a hash" do
    config = Ackro::Configurations.read(Str)
    config.should
    config.config.ordered.first.first.should == :defaults
    config.config.ordered.last.first.should  == :html
    config.config[:html].ordered.first.first.should == :body
  end
